...
router.post("/check", function(req, res, next) {
  Item.find({uid : req.body.id}).then(result =>
    {
      if(result.length > 0)
      {
        var element_json = result[0].toJSON();

        const selected_product = new Basket(
        {
          name : element_json.name,
          item : element_json.id
        }
        );

        selected_product.save(function(err)
        {
          if(err)
            ;
        });
